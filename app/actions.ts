...
export async function login(formData: FormData) {
  ...
  try {
    ...
    if (!apiUrl) {
      ...
      return redirect(
        `/admin/auth?error=${encodeURIComponent("Server configuration error.")}`
      );
    }
    ...
    if (!response.ok) {
      ...
      return redirect(`/admin/auth?error=${encodeURIComponent(errorMessage)}`);
    }
    ...
    return redirect(
      `/admin/auth?error=${encodeURIComponent("An unexpected error occurred.")}`
    );
  }
}
