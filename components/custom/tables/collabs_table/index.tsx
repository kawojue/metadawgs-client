...
import { Collabs, MetaType } from "@/lib/type";
...
export default function CollabsTable({ isPreview }: Props) {
  ...
  useEffect(() => {
    debouncedFetch(async (signal) => {
      ...
      setCollabs(resCollabs.data.data);
      setMeta(resCollabs.data.meta);

      return resCollabs.data;
    });
  ...
  return (
    <div className="space-y-3">
      <div className="flex sm:justify-between sm:flex-row flex-col-reverse gap-4 sm:items-center">
        <div className="flex items-center gap-3">
          <h2 className="text-2xl font-semibold font-fredoka">
            {isPreview && "New "}Collabs Requests
          </h2>
          {isPreview && (
      ...
      <div className="w-full space-y-8">
        <DataTable columns={columns} data={Collabs} isLoading={loading} />
        {meta && !loading && (
          <ShadcnPagination
            meta={meta}
            baseUrl={
              isPreview
                ...
                : "/wherethemagicrestricted/collabs"
            }
          />
        )}
      </div>
