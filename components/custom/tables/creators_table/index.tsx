...
import { Approval, MetaType } from "@/lib/type";
...
export default function CreatorsTable({ isPreview }: Props) {
  ...
  useEffect(() => {
    debouncedFetch(async (signal) => {
      const resApprovals = await fetchWithAuth<{
        data: Approval[];
        meta: MetaType;
      }>(`/users/approvals?page=${page}&limit=${limit}&search=${search}`, {
        isAdmin: true,
        signal,
      });

      setApprovals(resApprovals.data.data);
      setMeta(resApprovals.data.meta);

      return resApprovals.data;
    });
          ...
          <ShadcnPagination
            ...
            baseUrl={
              isPreview
                ...
                : "/wherethemagicrestricted/approvals"
            }
          />
        )}
      </div>
