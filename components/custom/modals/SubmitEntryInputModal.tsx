...
function SubmitEntryInputModal({
  ...
  const isValidLink = (url: string): boolean => {
    try {
      ...
      const isTwitter =
        ...
        /^\/\w+\/status\/\d+$/.test(parsedUrl.pathname);

      // Match for YouTube (long + short)
      const isYouTube =
        (hostname === "youtube.com" || hostname === "www.youtube.com") &&
        parsedUrl.searchParams.has("v"); // e.g. https://www.youtube.com/watch?v=abc123
      const isYouTubeShort =
        hostname === "youtu.be" && /^\/[\w-]+$/.test(parsedUrl.pathname); // e.g. https://youtu.be/abc123

      // Match for TikTok
      const isTikTok =
        (hostname === "tiktok.com" || hostname === "www.tiktok.com") &&
        /^\/@[\w.-]+\/video\/\d+$/.test(parsedUrl.pathname); // e.g. https://www.tiktok.com/@user/video/1234567890

      return (
        isHttpOrHttps && (isTwitter || isYouTube || isYouTubeShort || isTikTok)
      );
    } catch {
      return false;
    }
  ...
  useEffect(() => {
    if (link.trim() === "") {
      ...
      setValidationError(
        `Please enter a valid ${isMindShare ? "tweet or reel" : "tweet"} link.`
      );
    } else {
      setValidationError(null);
    }
